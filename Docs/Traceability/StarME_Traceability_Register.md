...
| Deploy-01 | Private server deployment | Non-root API image and isolated PostgreSQL/Redis/API/RQ Compose topology | Compose validation, migration head, live/ready checks, SSH-tunnel check, authenticated catalogue smoke test | Synthetic loopback deployment active on AIStaging; DNS/TLS and Legal consent version pending |
